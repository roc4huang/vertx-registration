...
def suite = TestSuite.create('de.thokari.vertx.registration.registrationTest')
...
suite.beforeEach({ context ->
    ...
    vertx.deployVerticle 'groovy:de.thokari.vertx.registration.registration', [ config: appConfig.registration ], { res ->
        if (!res.succeeded()) {
